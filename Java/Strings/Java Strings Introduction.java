...
public class Solution {
    ...
    private void solve(Printer p, int testcase) throws Exception {
        ...
        p.println(s.length()+ss.length());
        if((int)s.compareTo(ss)>0)  p.PYes() ; else p.PNo();
        p.println(s.substring(0, 1).toUpperCase() + s.substring(1),ss.substring(0, 1).toUpperCase() + ss.substring(1));
